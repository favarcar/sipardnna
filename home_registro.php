<h1 class="centrar letra n600 azulo pi">Menú Principal</h1>



<div class="row clearfix ps pi">
      <div class="col-lg-3"> 
	  <div class="col-md-12 col-sm-12 ps pi">
      <a href="main.php?key=8"><img src="iconos/ninos1.png" alt="..." width="100" height="100" class="center-block pic"></a>
      <h4 class="centrar letra azulo n400 ps"><strong>Registrar Ni&ntilde;os, Ni&ntilde;as o Adolescentes</strong></h4>
      </div>
      <div class="col-md-12 col-sm-12 ps pi">
      <a href="main.php?key=2"><img src="iconos/padres1.png" alt="..." width="100" height="100" class="center-block pic"></a>
      <h4 class="centrar letra azulo n400 ps"><strong>Registrar Madres, Padres o Cuidadores</strong></h4>
      </div>
	  </div>
	  <div class="col-lg-6">
	  <img width="100%" src="img/bg-1.png">
	  </div>

	  <div class="col-lg-3">
      <div class="col-md-12 col-sm-12 ps pi">
      <a href="main.php?key=12"><img src="iconos/expe1.png" alt="..." width="100" height="100" class="center-block pic"></a>
      <h4 class="centrar letra azulo n400 ps"><strong>Consultar Expedientes </strong></h4>
      </div>

      <div class="col-md-12 col-sm-12 ps pi">
      <a href="main.php?key=4"><img src="iconos/usuario1.png" alt="..." width="100" height="100" class="center-block pic"></a>
      <h4 class="centrar letra azulo n400 ps"><strong>Mi Usuario</strong></h4>
      </div>
      <div class="col-md-12 col-sm-12 ps pi">
      <a href="main.php?key=13" class="btn btn-primary"><i class="fa fa-exclamation-circle"class="center-block pic"></i> Notificaciones</a>
      <h4 class="centrar letra azulo n400 ps"></h4>
      </div>
   </div>
</div>
    </section>
