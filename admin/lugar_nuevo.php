...
<br>	
<center><h3 >Ingresar nuevo lugar de los hechos</h3></center>
<form action="main.php?key=88" method="post" enctype="multipart/form-data" name="nuevo_reg" target="_self" id="nuevo_reg">
  <?php do{?>
  <?php
...
<div class="table-responsive">
  ...
  <table id="datatable-buttons"  class="table table-striped table-bordered" align="center" border="1" style="width:auto; height:20px;" >
      <thead>
	<tr>
	  <td colspan="10" class="letra n600 azulo" bgcolor="#ff9933">Lugar de los hechos:
	  <!--Traer el numero de usuarios registrados en la tabla usuarios -->
	  <?php
                                                $con4 = mysqli_query($con, "SELECT count(ID_LUGAR) FROM lugar_hechos");
                                                while ($row4 = mysqli_fetch_array($con4)) {
                                                    echo $nom_asignatura11 = $row4['count(ID_LUGAR)'];
                                                } ?>
...
<div id="botones_com" style="float: left;">
   <!--boton ver-->
  <a href="main.php?key=89&Id=<?= $row_sql[$id] ?>"  class="btn btn-primary btn-sm" style="display:<?=$visible;?>"><span class="glyphicon glyphicon-search"></span></a>
	<!--boton editar-->
<a href="main.php?key=86&Id=<?php echo $row_sql[$id]; ?>" class="btn btn-success btn-sm"><span class="glyphicon glyphicon-edit"></span></a>	
<!--boton eliminar-->
