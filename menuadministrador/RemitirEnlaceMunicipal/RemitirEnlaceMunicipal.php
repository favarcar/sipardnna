...
<form name="form1" method="post" action="RemitirEnlaceMunicipal.php" id="cdr" >
  <center>
    <h3 class="centrar letra n600 azulo pi">Remitir Enlace Municipal</h3>
...
if($busca!=""){
$busqueda=mysql_query("SELECT * FROM usuarios where id_perfil = 4 and apellidos LIKE '%".$busca."%' OR numero_documento LIKE '%".$busca."%'");//cambiar nombre de la tabla de busqueda
...
$busqueda=mysql_query("SELECT * FROM usuarios where id_perfil = 4  order by apellidos ");//cambiar nombre de la tabla de busqueda
while($row=mysql_fetch_array($busqueda)){
